...
kwargs_lens_init = [{'theta_E': 1.2244481295926168, 'center_x': 0.025522205479803314, 'center_y': 0.011140509686173512, 'e1': -0.19639036672670346, 'gamma': 2.0, 'e2': -0.10324237094456128},\
    {'e1': 0.0, 'e2': 0.0}]
kwargs_source_init = [{'R_sersic': 0.1, 'n_sersic': 1., 'e1': 0, 'e2': 0, 'center_x': 0, 'center_y': 0}]
kwargs_lens_light_init = [{'e1': -0.053855027414991499, 'n_sersic': 6.9999814846978046, 'center_x': 0.024969021409959959,
                           'center_y': 0.041663764972781694, 'amp': 0.055903261837359101, 'R_sersic': 4.1727752698451974, 'e2': 0.030283868799814307}]
...
#filename='fit_result_PSF{0}_circlemask'.format(psfno)
...
lens_result, source_result, lens_light_result, ps_result, cosmo_result,\
chain_list, param_list, samples_mcmc, param_mcmc, dist_mcmc = pickle.load(open('fit_result_PSF{0}'.format(psfno),'rb'))
...
#plt.savefig('fig_PSF{0}_image0_circlemask.pdf'.format(psfno))
plt.show()
...
f.tight_layout()
f.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0., hspace=0.05)
#plt.savefig('fig_PSF{0}_image1_circlemask.pdf'.format(psfno))
plt.show()
...
#fig.savefig('fig_PSF{0}_corner_circlemask.pdf'.format(psfno))
plt.show()
