...
#%%
sort_label_type=['subg_2, PSFrecons', 'subg_3, PSFrecons']
...
for i in range(len(labels)):
    if labels[i][-17:] != sort_label_type[i%2]:
...
index0 = [i*6 for i in range(len(bars))]
label0 = [labels[i].split(',')[0] for i in index0]
index1 = [i*6+5 for i in range(len(bars))]
...
def plt_result(fixgamma, chisq=chisq):
    ...
    fmt =['--','-']
    lines_num = 2 #subg 2-3, PSFrecon and QSO mask
    texts = []
