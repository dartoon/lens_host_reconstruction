...
pyfits.PrimaryHDU(psf).writeto('PSF{0}_clean.fits'.format(len(psfs)),overwrite=True)  #Save Simon's PSF as PSF2
pyfits.PrimaryHDU(psf).writeto('PSF{0}_std.fits'.format(len(psfs)),overwrite=True) 

#%%
fig = profiles_compare(psfs+[psf], scal_list=np.ones(len(psfs)+1),
                       prf_name_list=['PSF'+str(i) for i in range(len(psfs)+1)],
