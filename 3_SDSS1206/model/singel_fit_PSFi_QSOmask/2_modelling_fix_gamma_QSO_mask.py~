...
if glob.glob('../fit_PSFi_QSOmask/{0}_stdd_boost.fits'.format(ID)) == []: 
    ...
    plt.show()
    pyfits.PrimaryHDU(lens_rms).writeto('../fit_PSFi_QSOmask/{0}_stdd_boost.fits'.format(ID),overwrite=False) 

lens_rms = pyfits.getdata('../fit_PSFi_QSOmask/{0}_stdd_boost.fits'.format(ID))
