...
stdd_boost_filename = old_folder+'/'+'{0}_stdd_boost.fits'.format(ID)
if glob.glob(stdd_boost_filename) == []: 
    ...
    for i in range(len(QSO_pos)):
        if i == 0:
            areas = (np.sqrt((QSO_pos[i][1]-xy_index[0])**2+(QSO_pos[i][0]-xy_index[1])**2) <6 )  # five piexls
        else:
            areas += (np.sqrt((QSO_pos[i][1]-xy_index[0])**2+(QSO_pos[i][0]-xy_index[1])**2) <6)  # five piexls
...
lens_rms = pyfits.getdata(stdd_boost_filename)
