...
ID = 'HS2209_vis05_redo'

#==============================================================================
# Cut out the lens image
#==============================================================================
fitsFile = pyfits.open('../data/HS2209+1914_F814W_vis05_redo_sci.fits')

from my_astro_func import read_pixel_scale
pixscl = read_pixel_scale('../data/HS2209+1914_F814W_vis05_redo_sci.fits')


# img = fitsFile[0].data # check the back grounp
# img[img==0.] = np.nan
# img_sub, bkg_light = sub_bkg(img)
# pyfits.PrimaryHDU(img_sub).writeto('../data/HS2209_vis05_redo_sublight.fits',overwrite=True)
img_sub = pyfits.getdata('../data/HS2209_vis05_redo_sublight.fits')
#%%
center_QSO = np.array([4888-1,5022-1]) #c_psf_list[QSO_loc]
...
wht = pyfits.getdata('../data/HS2209+1914_F814W_vis05_redo_wht.fits')
exp = read_fits_exp('../data/HS2209+1914_F814W_vis05_redo_sci.fits') #4542.0
...
plt.colorbar()
plt.show()
print("exptime:", np.mean(lens_exp_map))

#Note: The wht map looks interesting...
#plt.imshow(cut_image(wht, center_QSO, 100), norm=LogNorm(),origin='low')
#plt.colorbar()
#plt.show()
...
#PSFx, PSFy  = auto_cut_PSFs(img_sub)
 ...
 #%%
psfs_pos = np.array([[855, 3938], [4050, 3587], [4710, 4421], [4771, 5311], [4374, 5959],[4136, 6145]])  #The [4081, 5232] is too bright and saturated.
