...
framesize = 101
ct = int((len(lens_image) - framesize)/2)
...
if glob.glob('{0}_stdd_boost.fits'.format(ID)) == []: 
    ...
    for i in range(len(QSO_pos)):
        if i == 0:
            areas = (np.sqrt((QSO_pos[i][1]-xy_index[0])**2+(QSO_pos[i][0]-xy_index[1])**2) <4 )  # five piexls
        else:
            areas += (np.sqrt((QSO_pos[i][1]-xy_index[0])**2+(QSO_pos[i][0]-xy_index[1])**2) <4)  # five piexls
...
for fix_gamma in [[1.9, 2.0, 2.1][0]]:
    for psf_id in [3,4,0,1,2,5]:
        for subg in [2,3]:
            ...
            kwargs_lens_init = [{'center_x': -0.0692575310291098,
                               'center_y': 0.0685935396251733,
                               'e1': -0.20425841651443233,
                               'e2': -0.1091373417635464,
                               'gamma': 1.9,
                               'theta_E': 0.47696081746854574},
                              {'dec_0': 0,
                               'gamma1': -0.19332777277848875,
                               'gamma2': -0.1943182930328641,
                               'ra_0': 0}]
            kwargs_source_init = [{'R_sersic': 0.3083223807285265,
                                   'amp': 73.77742967833441,
                                   'center_x': -0.041405338059231814,
                                   'center_y': -0.06222145747182217,
                                   'e1': 0.08038888392894873,
                                   'e2': 0.34129068518053746,
                                   'n_sersic': 1.2866869420791776}]
            kwargs_lens_light_init =  [{'R_sersic': 5.056005849811609,
                                       'amp': 2.3681759143119483,
                                       'center_x': -0.020863579490008194,
                                       'center_y': 0.006736151513116011,
                                       'e1': -0.0989200881123484,
                                       'e2': -0.05833397614009003,
                                       'n_sersic': 4.976081719613192}]
            kwargs_ps_init = [{'dec_image': np.array([-0.5977922 ,  0.38773551]),
                              'ra_image': np.array([ 0.10488331, -0.22836423])}]   
