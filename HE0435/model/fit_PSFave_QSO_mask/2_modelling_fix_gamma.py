...
#filename='fit_result_PSFavestd_QSOmask_gammafix_subg{0}'.format(subg)
...
#lens_result, source_result, lens_light_result, ps_result, cosmo_result,\
#chain_list, param_list, samples_mcmc, param_mcmc, dist_mcmc = pickle.load(filename,'rb'))
...
plt.savefig('fig_PSFavestd_QSOmask_gammafix_subg{0}_img0.pdf'.format(subg))
plt.show()
...
f.tight_layout()
f.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0., hspace=0.05)
plt.savefig('fig_PSFavestd_QSOmask_gammafix_subg{0}_img1.pdf'.format(subg))
plt.show()
...
fig.savefig('fig_PSFavestd_QSOmask_gammafix_subg{0}_corner.pdf'.format(subg))
plt.show()

picklename='result_PSFavestd_QSOmask_gammafix_subg{0}'.format(subg)
fit_result = [lens_result, source_result, lens_light_result, ps_result, cosmo_result,chain_list, param_list, samples_mcmc, param_mcmc, dist_mcmc]
trans_result = [mcmc_new_list, labels_new]
pickle.dump([fit_result, trans_result], open(picklename, 'wb'))

import os
os.system('say "your program of avergaed PSF has finished"')