...
#==============================================================================
psfs_pos = np.array([[2855,1736],[2243,1475],[2621,4320],[3500,2394]]) #!!!!Need to be change
...
for i in range(len(psfs_pos)):
    print 'PSF',count
    PSF, PSF_center = cut_center_bright(image=img_sub, center=psfs_pos[i], radius=80,  return_center=True, plot=False, center_float=True)
    PSF_gauss_centers.append(PSF_center)
    PSF_exp_map =  cut_image(exp_map, PSF_center, 80)
    PSF_rms = (PSF/PSF_exp_map+stdd**2)**0.5
    _, PSF_br_center = cut_center_bright(image=img_sub, center=psfs_pos[i], radius=80, kernel = 'center_bright', return_center=True, plot=False)
