...
lens_image = pyfits.getdata('../SDSS0246_cutout.fits')   #!!!need to change
lens_rms = pyfits.getdata('../SDSS0246_stdd.fits')    #!!!need to change
...
exp_time = 8481.0 # exposure time (arbitrary units, flux per pixel is in units #photons/exp_time unit)
...
for i in range(len(QSOx)):
    if i == 0:
        areas = (np.sqrt((QSOx[i]-xy_index[1])**2+(QSOy[i]-xy_index[0])**2) <12 )  # 3 piexls
    else:
        areas += (np.sqrt((QSOx[i]-xy_index[1])**2+(QSOy[i]-xy_index[0])**2) <10 )  # 3 piexls
...
# Few things to set:
# =============================================================================
psfname_list = ['../PSF{0}.fits'.format(i) for i in range(3)]
psfs = []
...
#psfno = 0
#subg = 2
fname = 'bigmask_Shost_Slens_subg'
psfnos = [0,1,2]
subg_list = [3,2]
for psfno in psfnos:
    for subg in subg_list:
        ...
        kwargs_lens_init = [{'theta_E': 0.595705684235279, 'center_x': 0.03750008121373825, 'center_y': -0.0074994919864485405, 'e1': -0.00509997602735099, 'gamma': fix_gamma, 'e2': 0.05984059377321039}, {'dec_0': 0, 'ra_0': 0, 'e1': 0.03485619157844552, 'e2': 0.004571414628855993}]
        
        kwargs_source_init = [{'e1': 0.12444725856059109, 'n_sersic': 4.9472958637641105, 'center_x': -0.008551272273325528, 'center_y': 0.012192354528189606, 'amp': 1.662078902753553, 'R_sersic': 0.5052455465885309, 'e2': -0.49170911003868545}]
        
        kwargs_lens_light_init = [{'e1': 0.09579974365816668, 'n_sersic': 4.439495753754119, 'center_x': 0.03750008121373825, 'center_y': -0.0074994919864485405, 'amp': 3.928816335176527, 'R_sersic': 1.536712588906626, 'e2': -0.05729330723240766}]
        
        kwargs_ps_init = [{'ra_image': np.array([-0.59833034,  0.30272704]), 'dec_image': np.array([ 0.15057866, -0.47023133])}]
