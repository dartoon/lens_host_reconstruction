...
filename_p  = '../0_HE0435_color/SFH_*_PA00_param.fits'
...
fnu = np.array([456.43487, 16.11759, 48.60586])
lam = np.array([15418.99, 5373.10, 8084.25])
yerr_l = np.array([213.269/883.972, 213.269/883.972, 213.269/883.972]) #Assume the flux are at the same uncertainty.
mag = -2.5*np.log10(fnu) + 25
flambda = 10**(-0.4*(mag+2.402+5.0*np.log10(lam))) * 10**17
lam = lam/10000.
plt.scatter(lam, flambda, c='r', zorder = 100)
plt.errorbar(lam, flambda, yerr=yerr_l*flambda,fmt='.',color='gray',markersize=1,zorder = 99)
