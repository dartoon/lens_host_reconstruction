...
ken_wht = pyfits.getdata('wfi2033_wht_pow3_f160w_ext.fits')
from cut_image import cut_image
ken_wht = cut_image(ken_wht, [53,42], 42)
#plt.imshow(ken_wht, norm=LogNorm(),origin='lower')
#plt.show()
ken_wht = ken_wht/ken_wht.max()
lens_rms = lens_rms/ken_wht   #!!! Need to check if this is a right way to weight
...
# =============================================================================
fix_gamma = 2.
psfno =  'Kenspsf'#0, 1, 2, 5, 6, 7
subg = 2
fname = 'Shost_Dlenslight_UsingKenWeightmap'

psf = pyfits.getdata('../Kens_reconstruction/psf_wfi2033_f160w_latest_3x_cut189.fits')
#import rebin
#factor=3
#psf = rebin.block(psf,(len(psf)/factor,len(psf)/factor),factor=factor)
#psf = psf/psf.sum()
#psf0 = pyfits.getdata('../PSF0.fits')
#psf0 = psf0/psf0.sum()
#from flux_profile import profiles_compare
#f_ = profiles_compare([psf, psf0], scal_list=np.ones(2),
#                       prf_name_list=['Kens PSF', 'PSF0'],
...
kwargs_psf['kernel_point_source_subsampled'] = psf
kwargs_psf['point_source_subsampling_factor'] = 3
kwargs_psf['kernel_pixel_subsampled'] = psf
kwargs_psf['pixel_subsampling_factor'] = 3
...
kwargs_numerics = {'subgrid_res': subg, 'psf_subgrid': True}
