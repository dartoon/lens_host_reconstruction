...
\begin{abstract}
  One of the main challenges in using high redshift active galactic nuclei to study the correlations between the mass of the supermassive Black Hole (\mbh) and the properties of their active host galaxies is instrumental resolution. The magnification of strongly lensed quasars provide a way to effectively increase instrumental resolution and thus overcome this challenge. In this work, we study eight strong lens systems with deep {\it Hubble Space Telescope}(\hst) imaging, using the state-of-the-art \lenstronomy\ code to reconstruct the image of the source. Using the reconstructed brightness of the host galaxy, we infer the host galaxy stellar mass based on stellar population models. 
  \mbh\ are estimated from broad emission line using standard methods. Our results are in good agreement with recent work based on non-lensed AGN, providing additional evidence that the correlation evolve over cosmic time.  At the moment, the \pink{sample size of lensed AGN is small and thus} provide mostly a consistency check on systematic errors related to resolution for the non-lensed AGN. However, the number of known lensed AGN is expected to increase dramatically in the next few years, through dedicated searches in ground and space based wide field surveys, and may become a key diagnostic of black hole and galaxy co-evolution. 
\end{abstract}
...
%
The broad line properties of our samples are \pink{mostly} adopted from the literature. % \citep{Sluse2012, Peng2006, Shen2011}.
\pink{For SDSS1206, the line properties have been inferred by~\citet{Shen2011}. However, this system was investigated as a non-lensed AGN and the lensing magnification on the intrinsic continuum luminosity was not considered. We follow~\citet{Birrer2019} and apply the same magnification correction on the $\log(L_\lambda)$ in this work. For the rest of the lens sample except HS2209, their broad line properties are adopted from the literatures~\citep{Sluse2012, Peng2006} with lensing magnification effect considered. For HS2209, the spectrum data was observed from ESI in September 2015. We derived the line properties using the same approach as~\citep{Sluse2012}. \todo{Dominique and Chris can give more details here.} }. The \mbh\ measurements, together with the properties of the board-line are listed in Table~\ref{mbh}. 
...
{\bf HE0435} ~ In addition to the F160W data, this system has been observed by \hst\ through filters F814W and F555W. \pink{The multi-band imaging data provides us the opportunity to estimate the host color and derive an accurate stellar population. We adopted the lensing modelling approach as introduced in Section~\ref{sec:photometry} to infer the host photometry at F814W and F555W two bands. Based on the three band host magnitudes, we find a 1.5~Gyr age and solar metallicity stellar population could well match the color. More detailed information is presented in Appendix~\ref{app:HE0435}.}
...
\log \big( \frac{\mathcal M_{\rm BH}}{10^{7}M_{\odot}})= \alpha + \beta \log(\frac{M_*}{10^{10}M_{\odot}}),
  \end {eqnarray}
\pink{with calibrated value as  $\alpha = 0.27$, $\beta = 0.98$ based on the 55 local sample.} We find that our lensed systems are above the local relation, consistent with the inference from the 32  non-lensed AGNs published by D20 in a  similar redshift range. To quantify the evolution as a function of redshift, we parameterize it as:
\begin{eqnarray}
\label{eq:offset}
\Delta \log \mathcal M_{\rm BH}= \gamma \log (1 + z),
\end{eqnarray} 
where the $\Delta \log \mathcal M_{\rm BH}$ is the offset of \mbh\ with respect to the local baseline at fixed \mstar. To make a direct comparison, we reproduce the plot shown in Figure~8 of D20. Then, we add our new lensed AGN measurement and show the result in Figure~\ref{fig:scaling_relation}-(b). We find that the offset from the local relationship is indistinguishable for the two samples. \pink{Based on Equation~\eqref{eq:offset}, we estimate the evolution of the eight lensed systems and obtain $\gamma=1.49\pm0.43$, which overlaps with D20's inference ($\gamma=1.03\pm0.25$ using 32 AGN) within $1-\sigma$ level.} This consistency between the two measurements provides an important verification of the accuracy of the results and strengthens the conclusions drawn by D20\pink{that the observed value of \mbh\ at a fixed \mstar\ tend to be larger at higher redshift than the ones in the local universe.}

\pink{The lensed AGN systems in this work are directly taken from the H0LiCOW collaboration. We note that selection function of our sample is irrelevant to D20's sample. Given that the eight lensed AGN sample is relatively a small size, in this work we only take them for consistency check with the previous studies. The selection effect of the lensed sample will be investigated when the volume of the lens systems is large enough with statistical significance.}
...
(left): BH masses vs. stellar mass correlation (\mbh-\mstar). The black line and the gray shaded region indicate the best-fit and 1$\sigma$ confidence interval based on Equation~\ref{eq:MMlocal}.
(right): offset of $\log(\mathcal M_{\rm BH})$ (vs. \mstar) as a function of redshift. \pink{We plot our new measurements on top of the figure to make direct comparison. The red line and shaded region are the best-fit and $1-\sigma$ offset fitting by Equation~\eqref{eq:offset} using the 32 high-z AGNs in D20, while the blue line and region are the fitting based on the eight lensed systems.} \blue{[Xuheng: Let me know if your don't like the current plots.]}
}
\end{figure*} 

\section{Systematic Errors}\label{sec:diss}

We considered set of modelling choices to perform the fitting and the final inference is based on a weighting of top ranked choices. In particular, we treat the two different modelling approaches, i.e., {\it noise boost} and {\it PSF iteration} equally, to derive the averaged results. Of course, the results are somewhat dependent on the weighting scheme. However, the dependency is smaller than other sources of uncertainty.  For example, the dispersion of the results by the top ranked choices and thus using different weighting schemes would only change the results marginally ($<0.1$~dex).
...
We directly compare our sample to the recent measurements by~\citet[][D20]{Ding2020a}, who used the similar approaches to derive the \mstar\ and calibrated the \mbh\ using consistent recipes. The \mbh-\mstar\ correlation and its evolution with cosmic time is in excellent agreement with the results obtained by D20, as shown in Figure~\ref{fig:scaling_relation}. The consistent result by our sample confirms\pink{the conclusion of evolution of the D20 as follows. First of all, the growth of the supermassive black hole predates that of its host galaxy during their co-evolution. Besides, for the high-$z$ sample, it is the total stellar masses that are measured and compared. Note that the actual morphologies of these hosts could be more complicated, such as disk and bulge component (system RXJ1131 for example). However, the local sample are bulge-dominated galaxies and the entire local \mstar\ adopted is their bulge mass. If the high-$z$ sample could be all resolved to focus the comparison on the bulge, the compared \mstar\ would be smaller (i.e., disk mass removed). As a result, the inferred evolution for the \mbh-bulge relation would be much even stronger. Taken together, these results suggest a scenario in which the stellar mass is transferred from disk to the bulge at a faster rate than the growth of \mbh\ since $z\sim2$.}

Our work based on highly magnified AGN showcases the power of lensing to effectively increase the resolution of a telescope and shows that uncertainties related to lens modelling are subdominant with respect to other sources of uncertainty like black hole mass. Furthermore, our work provides a powerful verification of the fidelity of the host galaxy reconstruction in non-lensed AGN. 
...
\todo{Xuheng: Hi Tommaso please confirm if I cite the right reference including Treu2010, Auger2010, Tre++18, Ang++18, Lem++20}
...
Illustrations of the inference of HE0435 using the multi-band data. Top two panels: best-fit model of the lensed arcs shown as Figure~\ref{fig:image_inference}. Bottom panel: SED inference. \pink{The red points with error are the inferred host flux at the three bands in the image plane}.}
\end{figure*} 
...
\pink{In addition to the WFC3/F160W, WFI2033 has been observed by \hst\ in other four bands, including WFC3/F125W (GO-12874; PI: D. Floyd), WFC3/F140W (GO-13732; PI: A. Nierenberg), ACS/F555W+F814W  (GO-9744; PI: C. S. Kochanek). Similar to Section~\ref{app:HE0435}, the lens modelling and the host photometry of WFI2033 have been inferred through these four bands. The results are presented in the Figure~\ref{fig:app_WFI2033}-(a)$-$(d). Unfortunately, limited by its exposure time, the data quality through F125W is not ideal and the host information could not be well extracted. Thus, we does not take the inference by F125W into account in the SED fitting. Using four-band photometry, we find the stellar population of 0.625 Gyr age could well matched the color, i.e., Figure~\ref{fig:app_WFI2033}-(e).}
\begin{figure*}
\centering
...
Same as Figure~\ref{fig:app_WFI2033} but for WFI2033. \pink{The inference of the host photometry is not clear using the F125W data, and thus the SED fitting is perform without this band.}}
\end{figure*} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
