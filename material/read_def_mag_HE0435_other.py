...
for idx in range(len(filename)):
    ...
    if len(kwargs_result['kwargs_lens_light'])>1:
        ...
        #    #Cal Bulge and then disk:
            ...
            lens_profile = imageModel.image(kwargs_lens_light=kwargs_result_copy['kwargs_lens_light'], unconvolved=True)
    ...
    imageModel = ImageModel(data_class, psf_class, lens_light_model_class=lens_light_model_class,
                                    point_source_class=point_source_class, kwargs_numerics=kwargs_numerics)
    lens_profile = imageModel.image(kwargs_lens_light=kwargs_result['kwargs_lens_light'], unconvolved=True)
...
#%%
read_i = 2
weighted_value = np.mean(mag_result[:,read_i])
rms_value = np.std(mag_result[:,read_i])
print "{0:.2f}".format(round(weighted_value,2)), "+-" , " {0:.2f}".format(round(rms_value,2))
