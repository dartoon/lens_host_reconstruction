...
#    del QSOx[1]     #!!! In the next run, needs to save the QSO masks for 
#    del QSOy[1]
    xy_index = np.indices((numPix,numPix))
    for i in range(len(QSOx)):
        if i == 0:
            areas = (np.sqrt((QSOx[i]-xy_index[1])**2+(QSOy[i]-xy_index[0])**2) <4 )  #!!! pixel numbers  #12  #Need to set based on QSO_mask_RMS
        else:
            areas += (np.sqrt((QSOx[i]-xy_index[1])**2+(QSOy[i]-xy_index[0])**2) <4 )  #!!! pixel numbers #11
    lens_mask[areas != 0] = 0
    if 'mask' in filename:            
...
#    kwargs_psf = sim_util.psf_configure_simple(psf_type='PIXEL', kernelsize=len(psf), deltaPix=deltaPix,kernel=psf)
    ...
    from lenstronomy.Plots.model_plot import ModelPlot as LensModelPlot
...
#                                    point_source_class, kwargs_numerics=kwargs_numerics)
    ...
    kwargs_result = {'kwargs_lens': lens_result,
                                   'kwargs_source': source_result,
                                   'kwargs_lens_light': lens_light_result,
                                   'kwargs_ps': ps_result
                                 }  
    lensPlot = LensModelPlot(multi_band_list, kwargs_model, kwargs_result, likelihood_mask_list=[lens_mask])
    if plt_show == 1:    
